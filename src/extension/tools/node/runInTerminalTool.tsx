...
export class RunInTerminalTool extends Disposable implements ICopilotTool<IRunInTerminalParams> {
	...
	/**
	 * Determines if a command should wait for completion instead of running in background.
	 * Commands like analysis, testing, building, etc. should wait for results.
	 */
	private shouldWaitForCompletion(command: string): boolean {
		const commandLower = command.toLowerCase().trim();

		// Common patterns that should wait for completion
		const waitPatterns = [
			// Analysis/linting commands
			/\b(analyze|analysis|lint|check|verify|validate)\b/,
			// Testing commands
			/\b(test|spec|jest|mocha|pytest|unittest)\b/,
			// Build/compile commands
			/\b(build|compile|make|gradle|maven)\b/,
			// Package manager commands
			/\b(npm|yarn|pip|flutter|fvm)\s+(install|build|test|analyze|doctor|clean|pub get|pub upgrade)\b/,
			// Git commands (usually quick)
			/\bgit\s+(status|add|commit|push|pull|fetch|diff|log)\b/,
			// Directory listing and file operations
			/\b(ls|dir|cat|head|tail|grep|find|which|where)\b/,
			// System info commands
			/\b(ps|top|htop|free|df|du|uname|whoami|date|pwd)\b/,
		];

		// Patterns that should run in background (long-running processes)
		const backgroundPatterns = [
			/\b(serve|server|dev|start|run|watch|daemon)\b/,
			/\b(nodemon|webpack-dev-server|live-server)\b/,
		];

		// Check if it's a background command first
		if (backgroundPatterns.some(pattern => pattern.test(commandLower))) {
			return false;
		}

		// Check if it should wait for completion
		return waitPatterns.some(pattern => pattern.test(commandLower));
	}
	...
	async invoke(options: vscode.LanguageModelToolInvocationOptions<IRunInTerminalParams>, token: CancellationToken) {
		...
		// Override isBackground for common analysis/build commands that should wait for completion
		const shouldWaitForCompletion = this.shouldWaitForCompletion(command);
		const actualIsBackground = options.input.isBackground && !shouldWaitForCompletion;

		if (options.input.isBackground && shouldWaitForCompletion) {
			this.logService.logger.debug(`RunInTerminalTool: Overriding background mode for command that should wait: \`${command}\``);
		}
		...
		if (actualIsBackground) {
			...
			try {
				...
				this.sendTelemetry({
					...
					isBackground: actualIsBackground,
					error,
					outputLineCount: -1,
					exitCode: undefined,
					isNewSession: true,
